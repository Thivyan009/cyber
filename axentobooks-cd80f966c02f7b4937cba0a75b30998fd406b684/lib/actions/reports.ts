...
export async function getMonthlyTransactions(month: Date, businessId?: string) {
  try {
    ...
    if (!targetBusinessId) {
      console.log("No businessId provided, fetching first business for user:", session.user.id);
      const business = await prisma.business.findFirst({
        where: { userId: session.user.id },
      });

      if (!business) {
        console.error("No business found for user:", session.user.id);
        throw new Error("Business not found");
      }

      targetBusinessId = business.id;
      console.log("Found business:", { id: business.id, name: business.name });
    }
    ...
    if (!business) {
      console.error("Business not found:", targetBusinessId);
      throw new Error("Business not found");
    }
